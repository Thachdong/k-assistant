...
export const useGenerateAction = ({
  ...
  const [currentAction, setCurrentAction] = useState<TCurrentAction | null>(null);

  const onRefactor = useCallback(async (filePath: string, fileContent: string) => {
    ...
    while (true) {
      ...
      setCurrentAction({ prompt: `Refactor file $${filePath}`, stream: awnser });
    }
  }, []);
